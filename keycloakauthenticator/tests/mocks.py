...
class MockAsyncHTTPClient(SimpleAsyncHTTPClient):
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.hosts = {}
        self.oidc_config = json.load(pkg_resources.resource_stream(__name__, 'oidc_config.json'))
...
def setup_oauth_mock(client, host, access_token_path, user_path,
    ...
    def access_token(request):
        ...
        return {
            'id_token': jwt.encode(user, jwk, algorithm=jwk['alg']),
            'access_token': jwt.encode(user, jwk, algorithm=jwk['alg']),
